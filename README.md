...
这不是声称真实均值已经被精确测到分，而是一条确定的择点规则：

1. 四个报价组件的解析加权中心为 `164.742` 元；
2. 按截止后页面显示的 `12,368` 名网球拍报价者重新模拟，均值为 `164.7408` 元；
3. `164.74` 落在平滑效用最高值的 99% 近优平台 `164.64–164.78` 内；
4. 在近优平台内，选择最接近解析中心且避开整数拥挤的分币，因此得到 `164.74`。
...
|时间与信息来源|全活动人数|LOT 18 报价人数|在模型中的用途|
|---|---:|---:|---|
|2026-09-01 截止日当天，用户记录|194,411|12,281|当时决策可见信息|
|2026-09-02 截止后，活动页复核|195,702|12,368|主模型采用的最终页面人数 `N`|

这里必须区分全活动人数与单件物品人数。网球拍均值只由 LOT 18 的报价形成，所以主模型使用 `N=12,368`，而不是 195,702。
...
- 实际页面人数 `N=12,368`；
- 163.50–165.50 元逐分网格，180,000 次模拟；
- 指定报价表，600,000 次模拟；
- 固定随机种子以便复现；
