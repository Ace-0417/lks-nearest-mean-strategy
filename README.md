...
本项目把一次真实活动中的**单次报价决策**整理为可复核的研究案例：参与者需要预测所有报价的均值，同时避开整数价和 AI 建议可能造成的同价拥挤。仓库保存了模型、固定随机种子的模拟程序、结果表和敏感性分析，而不只公布一个孤立答案。

> [!IMPORTANT]
> 这是截至 2026-09-01 的模型快照，不是对真实最终报价分布的测量，也不保证中标。活动参与应遵守平台规则。

## 核心结果

在当前信息和模型假设下，单次报价的结构性建议是：

```text
164.74 元
```

- 基准模型的群体报价均值约为 **164.74 元**。
- 局部平滑后的结构性最优点为 **164.74 元**；`164.65–164.85` 属于基准近优平台。
- 只改变 AI 相关参数时，较宽的模型近优区间约为 `164.57–164.89`。
- `165.00` 虽然接近均值，但整数与 AI 焦点拥挤使其模型中标率约为 `0.0017%`，而附近非整数报价约为 `0.36%`。
...
1. **事实、用户先验与模型假设分开**：公开页面人数是事实；球友比例、AI 使用率等是先验或假设。
2. **结果可复现**：程序固定随机种子并输出 CSV。
3. **结论附带边界**：人数、AI 聚类、报价中心等都做了敏感性分析。
...
│   └── full-analysis.md
├── data/results/
│   ├── baseline_fine_grid.csv
│   ├── baseline_requested_quotes.csv
...
模型把参与者分为球友/非球友与 AI/非 AI 四类，并用共享 Dirichlet 权重描述 AI 答案的相关聚类。主引擎按轮抽取类型人数、AI 焦点和报价总和，再对更近报价与同价并列进行条件积分，从而减少纯逐人模拟的方差与计算量。

完整的博弈论解释、参数、单次报价比较和局限见 [完整分析](reports/full-analysis.md)。

## 如何解读结果

- 推荐值是参数化模型的决策输出，不是现实频率的置信区间。
